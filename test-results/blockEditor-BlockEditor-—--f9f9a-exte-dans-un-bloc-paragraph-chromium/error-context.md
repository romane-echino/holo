...
- Name: blockEditor.spec.ts >> BlockEditor — navigation et structure >> Édition simple : saisir du texte dans un bloc paragraph
- Location: tests/e2e/blockEditor.spec.ts:300:3

# Error details

```
Error: locator.click: Error: strict mode violation: locator('[data-block-type="paragraph"]').filter({ hasText: 'Paragraphe un.' }) resolved to 2 elements:
    1) <div data-block-id="b2" data-block-type="paragraph" class="group/block relative">…</div> aka locator('div').filter({ hasText: 'Paragraphe un.' }).nth(3)
    2) <div data-empty="false" class="outline-none" contenteditable="true" data-block-type="paragraph" data-placeholder="Commencez à taper…">Paragraphe un.</div> aka getByTestId('block-editor').getByText('Paragraphe un.')

Call log:
  - waiting for locator('[data-block-type="paragraph"]').filter({ hasText: 'Paragraphe un.' })
