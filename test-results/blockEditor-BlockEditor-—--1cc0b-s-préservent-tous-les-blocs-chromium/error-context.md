...
- Name: blockEditor.spec.ts >> BlockEditor — intégrité du document >> plusieurs conversions successives préservent tous les blocs
- Location: tests/e2e/blockEditor.spec.ts:316:3

# Error details

```
Error: locator.click: Error: strict mode violation: locator('[data-block-type="paragraph"]').filter({ hasText: 'Gamma' }) resolved to 2 elements:
    1) <div data-block-id="b3" data-block-type="paragraph" class="group/block relative">…</div> aka locator('div').filter({ hasText: 'Gamma' }).nth(3)
    2) <div data-empty="false" class="outline-none" contenteditable="true" data-block-type="paragraph" data-placeholder="Commencez à taper…">Gamma</div> aka getByTestId('block-editor').getByText('Gamma')

Call log:
  - waiting for locator('[data-block-type="paragraph"]').filter({ hasText: 'Gamma' })
